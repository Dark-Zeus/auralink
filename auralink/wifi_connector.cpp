...
void updateWifiUI(bool force, bool isConnected, int32_t rssi) {
    static lv_obj_t* ui_sd_WifiIcon = nullptr;
    ...
    uint32_t now = millis();
    if (now - lastUpdate < 500 && !force) {
        return;
    }
    lastUpdate = now;

    lv_color_t color = lv_color_hex(0xFF0070);
    // if connected, show icon with strength
    if (!isConnected || (rssi <= -100)) {
        color = lv_color_hex(0xFF0070);
    } else if (rssi != 0 && rssi <= -70) {
        color = lv_color_hex(0xFFFF00);
    } else if (rssi != 0 && rssi <= -50) {
        color = lv_color_hex(0x00FF1B);
    } else if (rssi != 0) {
        color = lv_color_hex(0x2095F6);
    } else {
        color = lv_color_hex(0xFF0070);
    }
